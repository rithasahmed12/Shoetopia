    ...
    if (userId && cart) {
      ...
      if (cart && cart.items) {
        cart.items.forEach(async (cartItem) => {
          ...
          console.log('cartTtoalPrice:',cartTotalPrice);
          ...
          else if (cartItem.product_id.category && cartItem.product_id.category.offer) {
            itemPrice -= (itemPrice * cartItem.product_id.category.offer.percentage / 100);
          }
...
const verifyPayment = async (req, res) => {
  try {
    ...
    if (hmacFormat == details.payment.razorpay_signature) {
      ...
      console.log("jdhsjdhsjdhIVDE ETHITTO");
